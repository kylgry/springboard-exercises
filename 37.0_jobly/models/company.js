...
class Company {
  ...
  static async findAllFiltered(filter={}) {
    ...
    if (maxEmployees) queryFilters.push(`num_employees <= ${maxEmployees}`)
    if (minEmployees) queryFilters.push(`num_employees >= ${minEmployees}`)
    if (name) queryFilters.push(`LOWER(name) LIKE '%${name.toLowerCase()}%'`)
